...
def test_reader_rejects_redirect_to_private_target(monkeypatch) -> None:
    reader=SafeHttpReader(NetworkPolicy(), resolver=lambda host, port, **kwargs: [(None,None,None,None,("93.184.216.34",port))])
    monkeypatch.setattr(reader, "_resolve", lambda host, port: ("93.184.216.34",) if host == "example.com" else ("10.0.0.1",))
    # A redirect is only accepted after the destination is resolved and revalidated.
    class FakeResponse:
        status=302
