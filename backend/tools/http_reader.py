...
class SafeHttpReader(Tool):
    metadata = ToolMetadata(
        name="http_reader",
        description="Read a bounded HTTPS response with DNS/IP and redirect policy enforcement",
        risk_level="medium",
        requires_network=True,
        requires_approval=True,
    )

    def __init__(self, policy: NetworkPolicy, *, resolver=None) -> None:
        self._policy = policy
    ...
    def read(self, url: str) -> dict[str, Any]:
        ...
        for redirect_count in range(self._policy.max_redirects + 1):
            ...
            # Revalidate the resolved addresses immediately before connecting.
